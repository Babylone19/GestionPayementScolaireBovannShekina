...
const DocumentPrinting: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // CORRECTION : Utiliser le même formatage que dans PaymentStats
        const studentsArray = extractStudentsFromResponse(studentsData);
        const paymentsArray = extractPaymentsFromResponse(paymentsData);
        ...
        paymentsArray.forEach(payment => {
          console.log(`Paiement ${payment.id}:`, {
            ...
            studentExists: studentsArray.some(s => s.id === payment.studentId)
          });
        });

        // Debug des étudiants sans correspondance
        studentsArray.forEach(student => {
          const studentPayments = paymentsArray.filter(p => p.studentId === student.id);
          if (studentPayments.length > 0) {
            console.log(`✅ Étudiant ${student.firstName} ${student.lastName}:`, {
              payments: studentPayments.length,
              validPayments: studentPayments.filter(p => p.status === 'VALID').length,
              statuses: studentPayments.map(p => p.status)
            });
          }
        });
        ...
        setDebugInfo(`
          ...
          Étudiants avec paiements VALID: ${studentsArray.filter(s => 
            paymentsArray.filter(p => p.studentId === s.id && p.status === 'VALID').length > 0
          ).length}
        `);
  ...
  const debugStudentPayments = useCallback((studentId: string) => {
    ...
    console.log(`🔍 DÉBOGAGE Étudiant ${studentId} (${student?.firstName} ${student?.lastName}):`, {
  ...
  const hasPayments = useCallback((studentId: string): boolean => {
    ...
    // Débogage pour les étudiants sans paiement VALID mais avec d'autres statuts
    if (!hasValidPayment && studentPayments.length > 0) {
      const student = students.find(s => s.id === studentId);
      console.log(`❌ Étudiant ${student?.firstName} ${student?.lastName} a des paiements mais pas VALID:`, 
        studentPayments.map(p => ({ status: p.status, amount: p.amount })));
    }
  ...
  const handleGenerateRegistrationForm = (studentId: string) => {
    ...
    // Implémentation PDF
    // window.open(`/api/print/registration/${studentId}`, '_blank');
  };
  ...
  const handleGenerateStudentCard = (studentId: string) => {
    ...
    // Implémentation PDF
    // window.open(`/api/print/idcard/${studentId}`, '_blank');
  };
  ...
  const handleBatchGenerate = (type: 'registration' | 'idcard') => {
    ...
    // Implémentation batch PDF
    // const idsParam = selectedStudents.join(',');
    // window.open(`/api/print/batch/${type}?ids=${idsParam}`, '_blank');
  };
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <div className="flex space-x-2 mt-2">
            ...
            >
              Voir paiements (console)
            </button>
            <button 
              onClick={() => console.log('Students:', students)}
              className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded"
            >
              Voir étudiants (console)
            </button>
          </div>
        </div>
